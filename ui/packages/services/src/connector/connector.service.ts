...
export class ConnectorService extends BaseService {
  ...
  public getConnectorInfo(connectorTypeId: string): Promise<ConnectorType> {
    this.logger?.info(
      "[ConnectorService] Getting the details of Connector:",
      connectorTypeId
    );
    const connectorType = connectorTypeId;
    const endpoint: string = this.endpoint(
      "/connector-types/debezium-connector-:connectorTypeId/:connectorType.json",
      true,
  ...
  public validateProperties(
    connectorTypeId: string,
    body: any
  ): Promise<PropertiesValidationResult> {
    this.logger?.info(
